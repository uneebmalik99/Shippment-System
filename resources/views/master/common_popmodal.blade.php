...
<div class="modal-body">
    <form method="POST" id="common_fields">
        <div class="common_section">
            <div class="add_data_section">
                ...
                <div class="input-group mb-3 after-add-more" style="border: 1px solid rgba(31, 104, 158, 0.26);
                    ...
                    <input type="text" name="country" value="{{ @$record[0]['country'] }}" class="form-control common_input" placeholder="{{@$placeholder['country']}}"
                        id="country" required>
                </div>
                <div class="input-group mb-3 after-add-more" style="border: 1px solid rgba(31, 104, 158, 0.26);
                    filter: drop-shadow(2px 2px 2px rgba(92, 174, 235, 0.55));display:flex;">
                    <input type="text" name="state" value="{{ @$record[0]['state'] }}" class="form-control common_input" placeholder="{{@$placeholder['state']}}"
                        id="state" required>
                </div>
                <div class="input-group mb-3 after-add-more" style="border: 1px solid rgba(31, 104, 158, 0.26);
                    filter: drop-shadow(2px 2px 2px rgba(92, 174, 235, 0.55));display:flex;">
                    <input type="text" name="port" value="{{ @$record[0]['port'] }}" class="form-control common_input" placeholder="{{@$placeholder['port']}}"
                    id="port" required>
                </div>
                <div class="input-group mb-3 after-add-more" style="border: 1px solid rgba(31, 104, 158, 0.26);
                    filter: drop-shadow(2px 2px 2px rgba(92, 174, 235, 0.55));display:flex;">
                    <input type="text" name="terminal" value="{{ @$record[0]['terminal'] }}" class="form-control common_input" placeholder="{{@$placeholder['terminal']}}"
        ...
        <div class="modal-footer">
            <button type="button" class="btn save_btn" id="record_add" value="{{ @$record[0]['id'] }}" tab="{{@$tab}}">
