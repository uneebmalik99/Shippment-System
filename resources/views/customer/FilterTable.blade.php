@if (@$user){
    ...
    @foreach ($user as $val)
        <tr style="border-bottom: 1.2px solid rgba(191, 191, 191, 1) !important">
            ...
            <td>
                @if (@$val['status'] == '1')
                    <div class="font-size badge badge-success py-1 px-2 rounded" style="font-weight:500;">Active</div>
                @else
                    <div class="font-size badge badge-danger py-1 px-2 rounded" style="font-weight:500;">In Active</div>
                @endif
            </td>
            {{-- @dd($val['created_at']) --}}
            ...
            <td>
                ...
                <a href={{ route('customer.profile') . '/' . @$val['id'] }}>
